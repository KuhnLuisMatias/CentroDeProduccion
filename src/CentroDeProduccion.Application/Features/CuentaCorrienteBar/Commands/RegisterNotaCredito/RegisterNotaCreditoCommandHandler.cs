...
/// <summary>
/// Appends a NotaCredito movement (negative Monto, reduces the bar's debt). The ledger is
...
public class RegisterNotaCreditoCommandHandler
{
        ...
        RegisterNotaCreditoCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            return Result.Failure<RegisterNotaCreditoResponse>(
                Error.Validation("MONTO_INVALIDO", "La nota de crédito debe tener monto negativo"));
        }
        ...
        var movimiento = new CentroDeProduccion.Domain.Entities.CuentaCorrienteBar
        {
            Id = Guid.NewGuid(),
            BarId = bar.Id,
            TipoMovimiento = TipoMovimientoCtaCteBar.NotaCredito,
            Monto = command.Monto,
